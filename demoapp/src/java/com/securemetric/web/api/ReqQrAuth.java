...
//        }
        ...
        WebResource service = client.resource ( APIController.getAuthURI()+  "/req/requestQrCode"  );
        ClientResponse response = service.accept ( MediaType.APPLICATION_JSON ).post ( ClientResponse.class , gson.toJson ( map ) );
         System.out.println("DEBUG Message[ReqQrAuth] : send param " + map.toString());
        System.out.println("DEBUG Message[ReqQrAuth] : call API" + response.toString());
        if(response.getStatus() == 200)
        {    
            authString = response.getEntity ( String.class ) ;  
            responseMap = gson.fromJson ( authString , HashMap.class );
            System.out.println("DEBUG Message[ReqQrAuth] : " + authString);
        }
           
        }catch (Exception e){
            System.out.println("Error");
             System.out.println("DEBUG Message[ReqQrAuth] : Failed with exception " + e.getMessage()); 
            e.printStackTrace();
        } 
