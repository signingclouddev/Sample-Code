...
private String strHMAC = null;
    public HashMap<String , String> Authenticate(String username, String password, String ipAddress, String userAgent) 
    {
        ...
        WebResource service = client.resource ( APIController.getAuthURI()+  "/req/requestSmsOtp"  );
        ClientResponse response = service.accept ( MediaType.APPLICATION_JSON ).post ( ClientResponse.class , gson.toJson ( map ) );
        System.out.println("DEBUG Message[ReqSMSAuth] : call API" + response.toString());
        ...
        {   
            ...
            System.out.println("DEBUG Message[ReqSMSAuth] : call API" + authString);
        }
           
        }catch (Exception e){
            System.out.println("Error");
             System.out.println("DEBUG Message[ReqSMSAuth] : Failed with exception " + e.getMessage()); 
            e.printStackTrace();
        } 
