...
@ManagedBean(name="crotpRequest")
@SessionScoped
public class CrotpRequest implements Serializable {
    ...
    public String getCrotpRequest() throws Exception {
        ...
        if ("0".equals(code)) {
            ...
            this.otpChallenge = jsonObject.get("otpChallenge").getAsString();
            FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("otpChallenge", otpChallenge);

            CrotpLogin crotpLogin = new CrotpLogin();
            crotpLogin.displayOtpChallenge();
            return null;
        } else {
