...
public class Adaptive implements Serializable {
    ...
    public String adaptiveAuth() throws Exception {
        ...
        if ("0".equals(code)) {
            ...
            this.devAccId = jsonObject.get("defAccId").getAsString();
            ...
            FacesContext.getCurrentInstance().getExternalContext().redirect("login-2FA.xhtml");
            return null;

        }  else if ("1".equals(code)) {
            FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("username", username);
            FacesContext.getCurrentInstance().getExternalContext().redirect("login-pwd.xhtml");
            return null;
