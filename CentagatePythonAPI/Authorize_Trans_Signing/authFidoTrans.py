...
hmacdata = str(username) + str(details) + str(transactionId) + str(fidoChallenge) + str(assertion) + str(authToken) +  str(integrationKey) + str(unixTimestamp) + str(ipAddress) + str(userAgent) + str(browserFp) + str(supportFido)
...
for url in ['https://<domain_name>/v2/CentagateWS/webresources/auth/authFidoTrans']:
    try:
