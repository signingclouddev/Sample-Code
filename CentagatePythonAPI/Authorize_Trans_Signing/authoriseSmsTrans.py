...
hmacdata = str(username) + str(details) + str(transactionId) + str(smsOtp) + str(authToken) +  str(integrationKey) + str(unixTimestamp) + str(ipAddress) + str(userAgent) + str(browserFp) + str(supportFido)
...
for url in ['https://<domain_name>/v2/CentagateWS/webresources/trans/authoriseSmsTrans/{username}']:
    try:
