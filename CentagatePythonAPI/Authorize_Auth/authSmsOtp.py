...
hmacdata = str(username) + str(smsOtp) + str(authToken) + str(integrationKey) + str(unixTimestamp) + str(supportFido) + str(ipAddress) + str(userAgent) + str(browserFp) 
...
for url in ['https://<domain_name>/v2/CentagateWS/webresources/auth/authSmsOtp']:
    try:
