...
hmacdata = str(username) + str(devAccId) + str(otp) + str(challenge) + str(details) + str(authToken) +  str(integrationKey) + str(unixTimestamp) + str(supportFido) + str(ipAddress) + str(userAgent) + str(browserFp)
...
for url in ['https://<domain_name>/v2/CentagateWS/webresources/auth/authQrCode']:
    try:
