...
hmacdata = str(username) + str(devAccId) + str(otp) + str(authToken) +  str(integrationKey) + str(unixTimestamp) + str(ipAddress) + str(userAgent) + str(browserFp) + str(supportFido)
...
for url in ['https://<domain_name>/v2/CentagateWS/webresources/auth/authOtp']:
    try:
